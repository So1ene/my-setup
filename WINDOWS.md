...
- Extract the file and put it into "project-name/wp-content/themes" folder. You can delete all the other default themes that come with wordpress. This themes folder is what you will want to push to github, so add a .gitignore file that will ignore all the other folders and files in `C:\wamp64\www\project-name` (ask us or search google for a wordpress .gitignore file)
